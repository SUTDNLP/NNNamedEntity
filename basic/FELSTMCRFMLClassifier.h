 ...
 *      Author: mszhang
 * LSTMCRFML, for word embedding (seperate one part for fine tune, set other part for not fine tune)
 */
...
public:
  ...
  inline void init(const NRMat<dtype>& wordEmb, const NRMat<dtype>& wordEmb_b, int wordcontext, const NRMat<dtype>& charEmb, int charcontext, const NRVec<NRMat<dtype> >& tagEmbs, int labelSize, int charhiddensize,
  ...
  inline dtype process(const vector<Example>& examples, int iter) {
    ...
    for (int count = 0; count < example_num; count++) {
      ...
      vector<Tensor<xpu, 2, dtype> > wordprime_f(seq_size), wordprimeLoss_f(seq_size);
      vector<Tensor<xpu, 2, dtype> > wordprime_nf(seq_size), wordprimeLoss_nf(seq_size);
      ...
      for (int idx = 0; idx < seq_size; idx++) {
        ...
        wordprime_f[idx] = NewTensor<xpu>(Shape2(1, _wordDim), d_zero);
        wordprimeLoss_f[idx] = NewTensor<xpu>(Shape2(1, _wordDim), d_zero);

        wordprime_nf[idx] = NewTensor<xpu>(Shape2(1, _wordDim), d_zero);
        wordprimeLoss_nf[idx] = NewTensor<xpu>(Shape2(1, _wordDim), d_zero);
      ...
      for (int idx = 0; idx < seq_size; idx++) {
        ...
        _words.GetEmb(words[0], wordprime_f[idx]);
        _words_b.GetEmb(words[0], wordprime_nf[idx]);


        concat(wordprime_f[idx], wordprime_nf[idx], wordprime[idx]);
        ...
        // char gated pooling
        _gatedchar_pooling.ComputeForwardScore(charhidden[idx], wordprime_f[idx], chargateweightMiddle[idx], chargateweight[idx], chargateweightsum[idx], chargatedpoolIndex[idx], chargatedpool[idx]);
        // tag prime get 
        if (_tagNum > 0) {
      ...
      if (_tagNum > 0) {
        for (int idx = 0; idx < seq_size; idx++) {
          ...
          unconcat(wordprimeLoss_f[idx], wordprimeLoss_nf[idx], wordprimeLoss[idx]);
        }
      }
      else {
        for (int idx = 0; idx < seq_size; idx++) {
      ...
      for (int idx = 0; idx < seq_size; idx++) {
        _gatedchar_pooling.ComputeBackwardLoss(charhidden[idx], wordprime_f[idx], chargateweightMiddle[idx], chargateweight[idx], chargateweightsum[idx],
            chargatedpoolIndex[idx], chargatedpool[idx], chargatedpoolLoss[idx], charhiddenLoss[idx], wordprimeLoss_f[idx]);
        //char convolution
      ...
      if (_words.bEmbFineTune()) {
        for (int idx = 0; idx < seq_size; idx++) {
          const Feature& feature = example.m_features[idx];
          const vector<int>& words = feature.words;
          _words.EmbLoss(words[0], wordprimeLoss_f[idx]);
        }
      }
      ...
      for (int idx = 0; idx < seq_size; idx++) {
        ...
        FreeSpace(&(wordprime_f[idx]));
        FreeSpace(&(wordprimeLoss_f[idx]));
        FreeSpace(&(wordprime_nf[idx]));
        FreeSpace(&(wordprimeLoss_nf[idx]));
  ...
  void predict(const vector<Feature>& features, vector<int>& results) {
    ...
    vector<Tensor<xpu, 2, dtype> > wordprime_f(seq_size);
    vector<Tensor<xpu, 2, dtype> > wordprime_nf(seq_size);

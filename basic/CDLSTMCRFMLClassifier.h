/*
 * CDLSTMCRFMLClassifier.h
 *
 *  Created on: Mar 18, 2015
 *      Author: mszhang
 */

#ifndef SRC_CDLSTMCRFMLClassifier_H_
#define SRC_CDLSTMCRFMLClassifier_H_
...
// class DLSTMCRFMLClassifier;

class CDLSTMCRFMLClassifier {
public:
  CDLSTMCRFMLClassifier() {
    _dropOut = 0.5;
  }
  ~CDLSTMCRFMLClassifier() {
...
public:
  ...
  inline dtype secondprocess(const vector<Example>& examples, const vector<Example>& last_examples, CDLSTMCRFMLClassifier last_classifier,  int iter) {
  ...
  void secondpredict(const vector<Feature>& features, const vector<Feature>& last_features, CDLSTMCRFMLClassifier last_classifier, vector<int>& results) {
  ...
  dtype secondcomputeScore(const Example& example, const Example& last_example, CDLSTMCRFMLClassifier last_classifier) {
  ...
  void secondcheckgrad(const vector<Example>& examples, const vector<Example>& last_subExamples, CDLSTMCRFMLClassifier last_classifier, Tensor<xpu, 2, dtype> Wd, Tensor<xpu, 2, dtype> gradWd, const string& mark, int iter) {
    int charseed = mark.length();
    for (int i = 0; i < mark.length(); i++) {
      charseed = (int) (mark[i]) * 5 + charseed;
    }
  ...
  void secondcheckgrads(const vector<Example>& examples, const vector<Example>& last_subExamples, CDLSTMCRFMLClassifier last_classifier, int iter) {
    ...
    secondcheckgrad(examples, last_subExamples, last_classifier, _crf_layer._tagBigram, _crf_layer._grad_tagBigram, "_crf_layer._tagBigram", iter);
...
#endif /* SRC_CDLSTMCRFMLClassifier_H_ */

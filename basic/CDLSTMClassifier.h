...
#ifndef SRC_CDLSTMClassifier_H_
#define SRC_CDLSTMClassifier_H_
...
public:
  ...
  inline void secondinit(const NRMat<dtype>& wordEmb, int wordcontext, const NRMat<dtype>& charEmb, int charcontext, const NRVec<NRMat<dtype> >& tagEmbs, int labelSize, int charhiddensize,
    ...
    _hiddensize = hiddensize;
    _lstmhiddensize = lstmhiddensize;
    _token_representation_size = 2 * _wordDim + _char_outputSize + _tag_outputSize;
    _inputsize = _wordwindow * _token_representation_size;
  ...
  inline dtype secondprocess(const vector<Example>& examples, const vector<Example>& last_examples, CDLSTMClassifier last_classifier,  int iter) {
    ...
    for (int count = 0; count < example_num; count++) {
      ...
      for (int idx = 0; idx < seq_size; idx++) {
        ...
        wordprime[idx] = NewTensor<xpu>(Shape2(1, 2 * _wordDim), d_zero);
        wordprimeLoss[idx] = NewTensor<xpu>(Shape2(1, 2 * _wordDim), d_zero);
        wordprimeMask[idx] = NewTensor<xpu>(Shape2(1, 2 * _wordDim), d_one);

        wordprime_l[idx] = NewTensor<xpu>(Shape2(1, _wordDim), d_zero);
        wordprimeLoss_l[idx] = NewTensor<xpu>(Shape2(1, _wordDim), d_zero);
  ...
  void secondpredict(const vector<Feature>& features, const vector<Feature>& last_features, CDLSTMClassifier last_classifier, vector<int>& results) {
  ...
  dtype secondcomputeScore(const Example& example, const Example& last_example, CDLSTMClassifier last_classifier) {
  ...
  void computeHiddenScore(const vector<Feature>& features, vector<Tensor<xpu, 2, dtype> >& hidden_vector) {
    ...
    for (int idx = 0; idx < seq_size; idx++) {
      ...
      else if (hidden_vector[idx].size(1) != _hiddensize) {
        std::cerr << "Hidden score calculation failed: Tenser.size(1) not match! " << hidden_vector[idx].size(1) << "VS" << _hiddensize << std::endl;
    ...
    for (int idx = 0; idx < seq_size; idx++) {
      for (int idy = 0; idy < _hiddensize; idy++) {
  ...
  void secondcheckgrad(const vector<Example>& examples, const vector<Example>& last_subExamples, CDLSTMClassifier last_classifier, Tensor<xpu, 2, dtype> Wd, Tensor<xpu, 2, dtype> gradWd, const string& mark, int iter) {
    int charseed = mark.length();
    for (int i = 0; i < mark.length(); i++) {
      charseed = (int) (mark[i]) * 5 + charseed;
    }
  ...
  void secondcheckgrads(const vector<Example>& examples, const vector<Example>& last_subExamples, CDLSTMClassifier last_classifier, int iter) {

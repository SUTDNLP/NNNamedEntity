...
#ifndef SRC_DLSTMClassifier_H_
#define SRC_DLSTMClassifier_H_
...
public:
  ...
  inline dtype secondprocess(const vector<Example>& examples, const vector<Example>& last_examples, DLSTMClassifier last_classifier,  int iter) {
    ...
    for (int count = 0; count < example_num; count++) {
      ...
      //forward propagation
      //input setting, and linear setting
      last_classifier.computeHiddenScore(last_example.m_features, wordprime);
  ...
  void secondpredict(const vector<Feature>& features, const vector<Feature>& last_features, DLSTMClassifier last_classifier, vector<int>& results) {
    ...
    //forward propagation
    //input setting, and linear setting
    last_classifier.computeHiddenScore(last_features, wordprime);
    for (int idx = 0; idx < seq_size; idx++) {
      const Feature& feature = features[idx];
      //linear features should not be dropped out
  ...
  dtype secondcomputeScore(const Example& example, const Example& last_example, DLSTMClassifier last_classifier) {
  ...
  void computeHiddenScore(const vector<Feature>& features, vector<Tensor<xpu, 2, dtype> >& hidden_vector) {
    ...
    for (int idx = 0; idx < seq_size; idx++) {
      if (hidden_vector[idx].size(0) != 1) {
        ...
        std::cerr << "Hidden score calculation failed: Tenser.size(1) not match! " << std::endl;
      }
      else {
  ...
  void secondcheckgrad(const vector<Example>& examples, const vector<Example>& last_subExamples, DLSTMClassifier last_classifier, Tensor<xpu, 2, dtype> Wd, Tensor<xpu, 2, dtype> gradWd, const string& mark, int iter) {
    int charseed = mark.length();
    for (int i = 0; i < mark.length(); i++) {
      charseed = (int) (mark[i]) * 5 + charseed;
    }
  ...
  void secondcheckgrads(const vector<Example>& examples, const vector<Example>& last_subExamples, DLSTMClassifier last_classifier, int iter) {

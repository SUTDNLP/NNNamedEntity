/*
 * CCDOLSTMClassifier.h
 *
 *  Created on: Mar 18, 2015
 *      Author: mszhang
 */

#ifndef SRC_CDOLSTMClassifier_H_
#define SRC_CDOLSTMClassifier_H_
...
public:
  ...
  inline void secondinit(const NRMat<dtype>& wordEmb, int wordcontext, const NRMat<dtype>& charEmb, int charcontext, const NRVec<NRMat<dtype> >& tagEmbs, int labelSize, int charhiddensize,
      int lstmhiddensize, int hiddensize, CDOLSTMClassifier last_classifier) {
    _wordcontext = wordcontext;
    _wordwindow = 2 * _wordcontext + 1;
    _wordSize = wordEmb.nrows();
    _wordDim = wordEmb.ncols();
    ...
    _hiddensize = hiddensize;
    _lstmhiddensize = lstmhiddensize;
    _token_representation_size = last_classifier._labelSize + _wordDim + _char_outputSize + _tag_outputSize;
    _inputsize = _wordwindow * _token_representation_size;
  ...
  inline dtype secondprocess(const vector<Example>& examples, const vector<Example>& last_examples, CDOLSTMClassifier last_classifier,  int iter) {
    assert(examples.size()==last_examples.size());
    _eval.reset();
    int example_num = examples.size();
    dtype cost = 0.0;
    int offset = 0;
    for (int count = 0; count < example_num; count++) {
      const Example& example = examples[count];
      const Example& last_example = last_examples[count];
      int seq_size = example.m_features.size();
      ...
      vector<Tensor<xpu, 2, dtype> > wordprime(seq_size), wordprimeLoss(seq_size), wordprimeMask(seq_size);
      vector<Tensor<xpu, 2, dtype> > wordrepresent(seq_size), wordrepresentLoss(seq_size);
      vector<Tensor<xpu, 2, dtype> > wordprime_l(seq_size), wordprimeLoss_l(seq_size), wordprimeMask_l(seq_size);
      vector<Tensor<xpu, 2, dtype> > wordprime_r(seq_size), wordprimeLoss_r(seq_size), wordprimeMask_r(seq_size);
      //initialize
      for (int idx = 0; idx < seq_size; idx++) {
        const Feature& feature = example.m_features[idx];
        ...
        wordprime[idx] = NewTensor<xpu>(Shape2(1, last_classifier._labelSize + _wordDim), d_zero);
        wordprimeLoss[idx] = NewTensor<xpu>(Shape2(1, last_classifier._labelSize+ _wordDim), d_zero);
        wordprimeMask[idx] = NewTensor<xpu>(Shape2(1, last_classifier._labelSize + _wordDim), d_one);

        wordprime_l[idx] = NewTensor<xpu>(Shape2(1, last_classifier._labelSize), d_zero);
        wordprimeLoss_l[idx] = NewTensor<xpu>(Shape2(1, last_classifier._labelSize), d_zero);

        wordprime_r[idx] = NewTensor<xpu>(Shape2(1, _wordDim), d_zero);
        wordprimeLoss_r[idx] = NewTensor<xpu>(Shape2(1, _wordDim), d_zero);
      ...
      //forward propagation
      //input setting, and linear setting
      last_classifier.computeHiddenScore(last_example.m_features, wordprime_l);
      ...
      for (int idx = 0; idx < seq_size; idx++) {
        ...
        const vector<int>& words = feature.words;
        _words.GetEmb(words[0], wordprime_r[idx]);

        concat(wordprime_l[idx], wordprime_r[idx], wordprime[idx]);
        ...
        _tanhchar_project.ComputeForwardScore(charinput[idx], charhidden[idx]);
        // char gated pooling
        _gatedchar_pooling.ComputeForwardScore(charhidden[idx], wordprime_r[idx], chargateweightMiddle[idx], chargateweight[idx], chargateweightsum[idx], chargatedpoolIndex[idx], chargatedpool[idx]);
        // tag prime get 
        if (_tagNum > 0) {
          const vector<int>& tags = feature.tags;
          for (int idy = 0; idy < _tagNum; idy++) {
      ...
      for (int idx = 0; idx < seq_size; idx++) {
        wordprimeLoss[idx] = wordprimeLoss[idx] * wordprimeMask[idx];
        unconcat(wordprimeLoss_l[idx], wordprimeLoss_r[idx], wordprimeLoss[idx]);
      }

      for (int idx = 0; idx < seq_size; idx++) {
        _gatedchar_pooling.ComputeBackwardLoss(charhidden[idx], wordprime_r[idx], chargateweightMiddle[idx], chargateweight[idx], chargateweightsum[idx],
            chargatedpoolIndex[idx], chargatedpool[idx], chargatedpoolLoss[idx], charhiddenLoss[idx], wordprimeLoss_r[idx]);
      ...
      if (_words.bEmbFineTune()) {
        for (int idx = 0; idx < seq_size; idx++) {
          const Feature& feature = example.m_features[idx];
          const vector<int>& words = feature.words;
          wordprimeLoss_r[idx] = wordprimeLoss_r[idx];
          _words.EmbLoss(words[0], wordprimeLoss_r[idx]);
        }
      }
      //tag fine tune
      if (_tagNum > 0) {
      ...
      for (int idx = 0; idx < seq_size; idx++) {
        ...
        FreeSpace(&(wordprime_l[idx]));
        FreeSpace(&(wordprimeLoss_l[idx]));
        FreeSpace(&(wordprime_r[idx]));
        FreeSpace(&(wordprimeLoss_r[idx]));
  ...
  void secondpredict(const vector<Feature>& features, const vector<Feature>& last_features, CDOLSTMClassifier last_classifier, vector<int>& results) {
    assert(features.size() == last_features.size());
    int seq_size = features.size();
    int offset = 0;
    ...
    vector<Tensor<xpu, 2, dtype> > wordprime(seq_size);
    vector<Tensor<xpu, 2, dtype> > wordrepresent(seq_size);
    vector<Tensor<xpu, 2, dtype> > wordprime_l(seq_size);
    vector<Tensor<xpu, 2, dtype> > wordprime_r(seq_size);
    ...
    //forward propagation
    //input setting, and linear setting
    last_classifier.computeHiddenScore(last_features, wordprime_l);
    for (int idx = 0; idx < seq_size; idx++) {
      const Feature& feature = features[idx];
      //linear features should not be dropped out
  ...
  dtype secondcomputeScore(const Example& example, const Example& last_example, CDOLSTMClassifier last_classifier) {
    assert(example.m_features.size() == last_example.m_features.size());
    int seq_size = example.m_features.size();
    int offset = 0;
  ...
  void computeHiddenScore(const vector<Feature>& features, vector<Tensor<xpu, 2, dtype> >& hidden_vector) {
    int seq_size = features.size();
    int offset = 0;
    if (hidden_vector.size()!= seq_size) {
      std::cerr << "Hidden score calculation failed: seq_size not match! " << std::endl;
    }
    ...
    //initialize
    for (int idx = 0; idx < seq_size; idx++) {
      if (hidden_vector[idx].size(0) != 1) {
        std::cerr << "Hidden score calculation failed: Tenser.size(0) not match! " << std::endl;
      }
      else if (hidden_vector[idx].size(1) != _labelSize) {
        std::cerr << "Hidden score calculation failed: Tenser.size(1) not match! " << hidden_vector[idx].size(1) << "VS" << _labelSize << std::endl;

      }
      else {
        // check format finished!
      }

      const Feature& feature = features[idx];

      int char_num = feature.chars.size();
      // int _tagNum = feature.tags.size();
      charprime[idx] = NewTensor<xpu>(Shape3(char_num, 1, _charDim), d_zero);
      charinput[idx] = NewTensor<xpu>(Shape3(char_num, 1, _char_inputSize), d_zero);
      charhidden[idx] = NewTensor<xpu>(Shape3(char_num, 1, _char_outputSize), d_zero);
      chargatedpoolIndex[idx] = NewTensor<xpu>(Shape3(char_num, 1, _char_outputSize), d_zero);
      ...
      project[idx] = NewTensor<xpu>(Shape2(1, _hiddensize), d_zero);
      output[idx] = NewTensor<xpu>(Shape2(1, _labelSize), d_zero);
      hidden_vector[idx] = NewTensor<xpu>(Shape2(1, _labelSize), d_zero);
    ...
    _tanh_project.ComputeForwardScore(lstmoutput, project);
    _olayer_linear.ComputeForwardScore(project, hidden_vector);

// normalize the hidden vector value
    dtype sum;
    for (int idx = 0; idx < seq_size; idx++) {
      sum = 0.0;
      for (int idy = 0; idy < _labelSize; idy++) {
        sum += exp(hidden_vector[idx][0][idy]);
      }
      for (int idy = 0; idy < _labelSize; idy++) {
        hidden_vector[idx][0][idy] = exp(hidden_vector[idx][0][idy])/sum;
      }
    }
  ...
  void updateParams(dtype nnRegular, dtype adaAlpha, dtype adaEps) {
    ...
    // softmax_updateAdaGrad(nnRegular, adaAlpha, adaEps);
  ...
  void checkgrads(const vector<Example>& examples, int iter) {
    ...
    // checkgrad(examples, softmax__tagBigram, softmax__grad_tagBigram, "softmax__tagBigram", iter);
  ...
  void secondcheckgrad(const vector<Example>& examples, const vector<Example>& last_subExamples, CDOLSTMClassifier last_classifier, Tensor<xpu, 2, dtype> Wd, Tensor<xpu, 2, dtype> gradWd, const string& mark, int iter) {
    int charseed = mark.length();
    for (int i = 0; i < mark.length(); i++) {
      charseed = (int) (mark[i]) * 5 + charseed;
    }
    ...
    for (int i = 0; i < examples.size(); i++) {
      Example oneExam = examples[i];
      lossAdd += secondcomputeScore(oneExam, last_subExamples[i], last_classifier);
    }
    ...
    for (int i = 0; i < examples.size(); i++) {
      Example oneExam = examples[i];
      lossPlus += secondcomputeScore(oneExam, last_subExamples[i], last_classifier);
    }
  ...
  void secondcheckgrads(const vector<Example>& examples, const vector<Example>& last_subExamples, CDOLSTMClassifier last_classifier, int iter) {
    secondcheckgrad(examples, last_subExamples, last_classifier, _olayer_linear._W, _olayer_linear._gradW, "_olayer_linear._W", iter);
    secondcheckgrad(examples, last_subExamples, last_classifier, _tanh_project._W, _tanh_project._gradW, "_tanh_project._W", iter);

    secondcheckgrad(examples, last_subExamples, last_classifier, _tanhchar_project._W, _tanhchar_project._gradW, "_tanhchar_project._W", iter);
    secondcheckgrad(examples, last_subExamples, last_classifier, _tanhchar_project._b, _tanhchar_project._gradb, "_tanhchar_project._b", iter);

    secondcheckgrad(examples, last_subExamples, last_classifier, _gatedchar_pooling._bi_gates._WL, _gatedchar_pooling._bi_gates._gradWL, "_gatedchar_pooling._bi_gates._WL", iter);
    secondcheckgrad(examples, last_subExamples, last_classifier, _gatedchar_pooling._bi_gates._WR, _gatedchar_pooling._bi_gates._gradWR, "_gatedchar_pooling._bi_gates._WR", iter);
    secondcheckgrad(examples, last_subExamples, last_classifier, _gatedchar_pooling._bi_gates._b, _gatedchar_pooling._bi_gates._gradb, "_gatedchar_pooling._bi_gates._b", iter);

    secondcheckgrad(examples, last_subExamples, last_classifier, _gatedchar_pooling._uni_gates._W, _gatedchar_pooling._uni_gates._gradW, "_gatedchar_pooling._uni_gates._W", iter);
    secondcheckgrad(examples, last_subExamples, last_classifier, _gatedchar_pooling._uni_gates._b, _gatedchar_pooling._uni_gates._gradb, "_gatedchar_pooling._uni_gates._b", iter);

    secondcheckgrad(examples, last_subExamples, last_classifier, rnn_left_project._lstm_output._W1, rnn_left_project._lstm_output._gradW1, "rnn_left_project._lstm_output._W1", iter);
    secondcheckgrad(examples, last_subExamples, last_classifier, rnn_left_project._lstm_output._W2, rnn_left_project._lstm_output._gradW2, "rnn_left_project._lstm_output._W2", iter);
    secondcheckgrad(examples, last_subExamples, last_classifier, rnn_left_project._lstm_output._W3, rnn_left_project._lstm_output._gradW3, "rnn_left_project._lstm_output._W3", iter);
    secondcheckgrad(examples, last_subExamples, last_classifier, rnn_left_project._lstm_output._b, rnn_left_project._lstm_output._gradb, "rnn_left_project._lstm_output._b", iter);
    secondcheckgrad(examples, last_subExamples, last_classifier, rnn_left_project._lstm_input._W1, rnn_left_project._lstm_input._gradW1, "rnn_left_project._lstm_input._W1", iter);
    secondcheckgrad(examples, last_subExamples, last_classifier, rnn_left_project._lstm_input._W2, rnn_left_project._lstm_input._gradW2, "rnn_left_project._lstm_input._W2", iter);
    secondcheckgrad(examples, last_subExamples, last_classifier, rnn_left_project._lstm_input._W3, rnn_left_project._lstm_input._gradW3, "rnn_left_project._lstm_input._W3", iter);
    secondcheckgrad(examples, last_subExamples, last_classifier, rnn_left_project._lstm_input._b, rnn_left_project._lstm_input._gradb, "rnn_left_project._lstm_input._b", iter);
    secondcheckgrad(examples, last_subExamples, last_classifier, rnn_left_project._lstm_forget._W1, rnn_left_project._lstm_forget._gradW1, "rnn_left_project._lstm_forget._W1", iter);
    secondcheckgrad(examples, last_subExamples, last_classifier, rnn_left_project._lstm_forget._W2, rnn_left_project._lstm_forget._gradW2, "rnn_left_project._lstm_forget._W2", iter);
    secondcheckgrad(examples, last_subExamples, last_classifier, rnn_left_project._lstm_forget._W3, rnn_left_project._lstm_forget._gradW3, "rnn_left_project._lstm_forget._W3", iter);
    secondcheckgrad(examples, last_subExamples, last_classifier, rnn_left_project._lstm_forget._b, rnn_left_project._lstm_forget._gradb, "rnn_left_project._lstm_forget._b", iter);
    secondcheckgrad(examples, last_subExamples, last_classifier, rnn_left_project._lstm_cell._WL, rnn_left_project._lstm_cell._gradWL, "rnn_left_project._lstm_cell._WL", iter);
    secondcheckgrad(examples, last_subExamples, last_classifier, rnn_left_project._lstm_cell._WR, rnn_left_project._lstm_cell._gradWR, "rnn_left_project._lstm_cell._WR", iter);
    secondcheckgrad(examples, last_subExamples, last_classifier, rnn_left_project._lstm_cell._b, rnn_left_project._lstm_cell._gradb, "rnn_left_project._lstm_cell._b", iter);


    secondcheckgrad(examples, last_subExamples, last_classifier, rnn_right_project._lstm_output._W1, rnn_right_project._lstm_output._gradW1, "rnn_right_project._lstm_output._W1", iter);
    secondcheckgrad(examples, last_subExamples, last_classifier, rnn_right_project._lstm_output._W2, rnn_right_project._lstm_output._gradW2, "rnn_right_project._lstm_output._W2", iter);
    secondcheckgrad(examples, last_subExamples, last_classifier, rnn_right_project._lstm_output._W3, rnn_right_project._lstm_output._gradW3, "rnn_right_project._lstm_output._W3", iter);
    secondcheckgrad(examples, last_subExamples, last_classifier, rnn_right_project._lstm_output._b, rnn_right_project._lstm_output._gradb, "rnn_right_project._lstm_output._b", iter);
    secondcheckgrad(examples, last_subExamples, last_classifier, rnn_right_project._lstm_input._W1, rnn_right_project._lstm_input._gradW1, "rnn_right_project._lstm_input._W1", iter);
    secondcheckgrad(examples, last_subExamples, last_classifier, rnn_right_project._lstm_input._W2, rnn_right_project._lstm_input._gradW2, "rnn_right_project._lstm_input._W2", iter);
    secondcheckgrad(examples, last_subExamples, last_classifier, rnn_right_project._lstm_input._W3, rnn_right_project._lstm_input._gradW3, "rnn_right_project._lstm_input._W3", iter);
    secondcheckgrad(examples, last_subExamples, last_classifier, rnn_right_project._lstm_input._b, rnn_right_project._lstm_input._gradb, "rnn_right_project._lstm_input._b", iter);
    secondcheckgrad(examples, last_subExamples, last_classifier, rnn_right_project._lstm_forget._W1, rnn_right_project._lstm_forget._gradW1, "rnn_right_project._lstm_forget._W1", iter);
    secondcheckgrad(examples, last_subExamples, last_classifier, rnn_right_project._lstm_forget._W2, rnn_right_project._lstm_forget._gradW2, "rnn_right_project._lstm_forget._W2", iter);
    secondcheckgrad(examples, last_subExamples, last_classifier, rnn_right_project._lstm_forget._W3, rnn_right_project._lstm_forget._gradW3, "rnn_right_project._lstm_forget._W3", iter);
    secondcheckgrad(examples, last_subExamples, last_classifier, rnn_right_project._lstm_forget._b, rnn_right_project._lstm_forget._gradb, "rnn_right_project._lstm_forget._b", iter);
    secondcheckgrad(examples, last_subExamples, last_classifier, rnn_right_project._lstm_cell._WL, rnn_right_project._lstm_cell._gradWL, "rnn_right_project._lstm_cell._WL", iter);
    secondcheckgrad(examples, last_subExamples, last_classifier, rnn_right_project._lstm_cell._WR, rnn_right_project._lstm_cell._gradWR, "rnn_right_project._lstm_cell._WR", iter);
    secondcheckgrad(examples, last_subExamples, last_classifier, rnn_right_project._lstm_cell._b, rnn_right_project._lstm_cell._gradb, "rnn_right_project._lstm_cell._b", iter);

    // secondcheckgrad(examples, last_subExamples, last_classifier, softmax__tagBigram, softmax__grad_tagBigram, "softmax__tagBigram", iter);

    secondcheckgrad(examples, last_subExamples, last_classifier, _words._E, _words._gradE, "_words._E", iter, _words._indexers);
    secondcheckgrad(examples, last_subExamples, last_classifier, _chars._E, _chars._gradE, "_chars._E", iter, _chars._indexers);
    // tag checkgrad
    for (int i = 0; i < _tagNum; i++){
      secondcheckgrad(examples, last_subExamples, last_classifier, _tags[i]._E, _tags[i]._gradE, "_tags._E", iter, _tags[i]._indexers);
    }
...
#endif /* SRC_CCDOLSTMClassifier_H_ */

 ...
 *      Author: mszhang
 *  Double layered LSTM labeler
 */

#include "CDOLSTMLabeler.h"
...
void Labeler::last_extractFeature(Feature& feat, const Instance* pInstance, int idx) {
  feat.clear();
  const vector<string>& words = pInstance->words;
  int sentsize = words.size();
  string curWord = idx >= 0 && idx < sentsize ? normalize_to_lowerwithdigit(words[idx]) : nullkey;
  // word features
  int unknownId = last_wordAlphabet.from_string(unknownkey);

  int curWordId = last_wordAlphabet.from_string(curWord);
  if (curWordId >= 0)
    feat.words.push_back(curWordId);
  else
    feat.words.push_back(unknownId);
  ...
  const vector<vector<string> > &tagfeatures = pInstance->tagfeatures;
  int tagNum = tagfeatures[idx].size();
  if (tagNum == last_tagAlphabets.size()){
    for (int i = 0; i < tagNum; i++) {
      unknownId = last_tagAlphabets[i].from_string(unknownkey);
      int curTagId = last_tagAlphabets[i].from_string(tagfeatures[idx][i]);
      if (curTagId >= 0)
        feat.tags.push_back(curTagId);
      else
        feat.tags.push_back(unknownId);
  ...
  // char features
  unknownId = last_charAlphabet.from_string(unknownkey);
  ...
  // actually we support a max window of m_options.charcontext = 2
  if (last_charAlphabet.size() >= 0) {
    for (int i = 0; i < cur_char_size; i++) {
      string curChar = cur_chars[i];

      int curCharId = last_charAlphabet.from_string(curChar);
      if (curCharId >= 0)
        feat.chars.push_back(curCharId);
      else
        feat.chars.push_back(unknownId);
    }
    int nullkeyId = last_charAlphabet.from_string(nullkey);
    if (feat.chars.empty()) {
      feat.chars.push_back(nullkeyId);
    }
  }
  ...
  const vector<string>& linear_features = pInstance->sparsefeatures[idx];
  feat.linear_features.clear();
  // for (int i = 0; i < linear_features.size(); i++) {
  //   int curFeatId = last_featAlphabet.from_string(linear_features[i]);
  //   if (curFeatId >= 0)
  //     feat.linear_features.push_back(curFeatId);
  // }
}
...
void Labeler::last_convert2Example(const Instance* pInstance, Example& exam) {
  exam.clear();
  const vector<string> &labels = pInstance->labels;
  int curInstSize = labels.size();
  for (int i = 0; i < curInstSize; ++i) {
    ...
    int numLabel1s = last_labelAlphabet.size();
    vector<int> curlabels, curlabel2s;
    for (int j = 0; j < numLabel1s; ++j) {
      string str = last_labelAlphabet.from_id(j);
      if (str.compare(orcale) == 0)
        curlabels.push_back(1);
      else
        curlabels.push_back(0);
    ...
    exam.m_labels.push_back(curlabels);
    Feature feat;
    last_extractFeature(feat, pInstance, i);
    exam.m_features.push_back(feat);
  }
}
...
void Labeler::last_initialExamples(const vector<Instance>& vecInsts, vector<Example>& vecExams) {
  int numInstance;
  for (numInstance = 0; numInstance < vecInsts.size(); numInstance++) {
    const Instance *pInstance = &vecInsts[numInstance];
    Example curExam;
    last_convert2Example(pInstance, curExam);
    vecExams.push_back(curExam);
    if ((numInstance + 1) % m_options.verboseIter == 0) {
      cout << numInstance + 1 << " ";
      if ((numInstance + 1) % (40 * m_options.verboseIter) == 0)
...
void Labeler::train(const string& trainFile, const string& devFile, const string& testFile, const string& modelFile, const string& optionFile,
    const string& wordEmbFile, const string& charEmbFile, const string& lastModelFile) {
  std::cout << "second_LSTM = " << second_LSTM << std::endl;
  if (second_LSTM) {
    loadLastModelFile(lastModelFile);
  }
  if (optionFile != "")
  ...
  // m_classifier.secondinit(wordEmb, m_options.wordcontext, charEmb, m_options.charcontext, tagEmbs, m_labelAlphabet.size(), m_options.charhiddenSize, m_options.rnnHiddenSize, m_options.hiddenSize);
  ...
  if (!second_LSTM) {
    ...
    m_classifier.setTagEmbFinetune(m_options.tagEmbFineTune);
    m_classifier.setDropValue(m_options.dropProb);
    last_classifier.setWordEmbFinetune(m_options.wordEmbFineTune);
    last_classifier.init(wordEmb, m_options.wordcontext, charEmb, m_options.charcontext, tagEmbs, m_labelAlphabet.size(), m_options.charhiddenSize, m_options.rnnHiddenSize, m_options.hiddenSize);
    last_classifier.setTagEmbFinetune(m_options.tagEmbFineTune);
    last_classifier.setDropValue(m_options.dropProb); 
  } else {
    m_classifier.setWordEmbFinetune(m_options.wordEmbFineTune);
    m_classifier.secondinit(wordEmb, m_options.wordcontext, charEmb, m_options.charcontext, tagEmbs, m_labelAlphabet.size(), m_options.charhiddenSize, m_options.rnnHiddenSize, m_options.hiddenSize, last_classifier);
    m_classifier.setTagEmbFinetune(m_options.tagEmbFineTune);
    m_classifier.setDropValue(m_options.dropProb);
  }
  ...
  static vector<Example> last_subExamples;
  vector<Example> last_trainExamples, last_devExamples, last_testExamples;
  vector<int> last_otherInstNums(otherInsts.size());
  vector<vector<Example> > last_otherExamples(otherInsts.size());
  if (second_LSTM) {
    last_initialExamples(trainInsts, last_trainExamples);
    last_initialExamples(devInsts, last_devExamples);
    last_initialExamples(testInsts, last_testExamples);
    for (int idx = 0; idx < otherInsts.size(); idx++) {
      last_initialExamples(otherInsts[idx], last_otherExamples[idx]);
      last_otherInstNums[idx] = last_otherExamples[idx].size();
    }
  }
  for (int iter = 0; iter < m_options.maxIter; ++iter) {
    ...
    for (int updateIter = 0; updateIter < batchBlock; updateIter++) {
      ...
      for (int idy = start_pos; idy < end_pos; idy++) {
        ...
        if (second_LSTM) {
          last_subExamples.push_back(last_trainExamples[indexes[idy]]);
        }
      }
      ...
      if (!second_LSTM) {
        ...
        cost = m_classifier.secondprocess(subExamples, last_subExamples, last_classifier, curUpdateIter);
      }
      ...
      if ((curUpdateIter + 1) % m_options.verboseIter == 0) {
        ...
        // } else {
        //   m_classifier.secondcheckgrads(subExamples, last_subExamples, last_classifier, curUpdateIter+1);
        // }
    ...
    if (devNum > 0) {
      ...
      for (int idx = 0; idx < devExamples.size(); idx++) {
        ...
        if (second_LSTM) {
          fullpredict(devExamples[idx].m_features, last_devExamples[idx].m_features, result_labels, devInsts[idx].words);
        } else {
      ...
      if (testNum > 0) {
        ...
        for (int idx = 0; idx < testExamples.size(); idx++) {
          vector<string> result_labels;
          if (second_LSTM) {
            fullpredict(testExamples[idx].m_features, last_testExamples[idx].m_features, result_labels, testInsts[idx].words);
          }
          else {
      ...
      for (int idx = 0; idx < otherExamples.size(); idx++) {
        ...
        for (int idy = 0; idy < otherExamples[idx].size(); idy++) {
          vector<string> result_labels;
          if (second_LSTM) {
            fullpredict(otherExamples[idx][idy].m_features, last_otherExamples[idx][idy].m_features, result_labels, otherInsts[idx][idy].words);
          }
          else {
      ...
      if (m_options.saveIntermediate && metric_dev.getAccuracy() > bestDIS) {
        ...
        if (!second_LSTM){
          writeLastModelFile(lastModelFile);
        } else {
...
int Labeler::fullpredict(const vector<Feature>& features, const vector<Feature>& last_features, vector<string>& outputs, const vector<string>& words) {
  assert(features.size() == words.size());
  vector<int> labelIdx, label2Idx;
  m_classifier.secondpredict(features,last_features, last_classifier, labelIdx);
  outputs.clear();
...
void Labeler::test(const string& testFile, const string& outputFile, const string& modelFile) {
  ...
  vector<Example> last_testExamples;
  last_initialExamples(testInsts, last_testExamples);
...
void Labeler::readWordEmbeddings(const string& inFile, NRMat<dtype>& wordEmb) {
  ...
  if (!bHasUnknown) {
    for (int idx = 0; idx < wordDim; idx++) {
      wordEmb[unknownId][idx] = 0.0; // sum[idx] / count;
    }
    count++;
...
void Labeler::enableSecondLSTM() {
  second_LSTM = true;
}

void Labeler::disableSecondLSTM() {
  second_LSTM = false;
}

void Labeler::loadLastModelFile(const string& inputModelFile) {
  std::cout << "Start load model from file: " << inputModelFile << std::endl;
  LStream inf(inputModelFile, "rb");
  last_options.loadModel(inf);
  last_options.showOptions();
  last_wordAlphabet.loadModel(inf);
  last_charAlphabet.loadModel(inf);
  last_labelAlphabet.loadModel(inf);
  last_featAlphabet.loadModel(inf);
  last_classifier.loadModel(inf);

  int last_tagAlphabets_size;
  ReadBinary(inf, last_tagAlphabets_size);
  last_tagAlphabets.resize(last_tagAlphabets_size);
  for (int idx = 0; idx < last_tagAlphabets_size; idx++) {
    last_tagAlphabets[idx].loadModel(inf);
  }
...
void Labeler::writeLastModelFile(const string & outputModelFile) {
...
void Labeler::writeModelFile(const string & outputModelFile) {
  ...
  last_options.writeModel(outf);
  last_wordAlphabet.writeModel(outf);
  last_charAlphabet.writeModel(outf);
  last_labelAlphabet.writeModel(outf);
  last_featAlphabet.writeModel(outf);
  last_classifier.writeModel(outf);

  int last_tagAlphabets_size = last_tagAlphabets.size();
  WriteBinary(outf, last_tagAlphabets_size);
  for (int idx = 0; idx < last_tagAlphabets_size; idx++) {
    last_tagAlphabets[idx].writeModel(outf);
  }
...
int main(int argc, char* argv[]) {
  ...
  std::string trainFile = "", devFile = "", testFile = "", modelFile = "", lastModelFile = "";
  std::string wordEmbFile = "", charEmbFile = "", optionFile = "";
  std::string outputFile = "";
  bool bTrain = false;
  bool preTrain = false;
  dsr::Argument_helper ah;
  ah.new_flag("p", "pretrain", "preTrain or not", preTrain);
  ...
  ah.new_named_string("lastmodel", "lastmodelFile", "lastnamed_string", "lastmodel file, must when training and testing", lastModelFile);
  ...
  Labeler tagger;
  if (preTrain) {
    tagger.disableSecondLSTM();
    tagger.train(trainFile, devFile, testFile, modelFile, optionFile, wordEmbFile, charEmbFile, lastModelFile);
  } 
  else if (bTrain) {
    tagger.enableSecondLSTM();
    tagger.train(trainFile, devFile, testFile, modelFile, optionFile, wordEmbFile, charEmbFile, lastModelFile);
  }
  else {
    tagger.enableSecondLSTM();

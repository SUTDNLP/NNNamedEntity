...
int Labeler::createAlphabet(const vector<Instance>& vecInsts) {
  ...
  for (int i = 0; i < tagNum; i++) {
	...
	for (feat_iter = word_stat.begin(); feat_iter != word_stat.end();
			feat_iter++) {
		if (!m_options.wordEmbFineTune
				|| feat_iter->second > m_options.wordCutOff) {
			m_wordAlphabet.from_string(feat_iter->first);
		}
	}

	for (feat_iter = char_stat.begin(); feat_iter != char_stat.end();
			feat_iter++) {
		if (!m_options.charEmbFineTune
				|| feat_iter->second > m_options.charCutOff) {
...
void Labeler::extractFeature(Feature& feat, const Instance* pInstance,
		int idx) {
	feat.clear();
...
void Labeler::train(const string& trainFile, const string& devFile,
		...
		const string& optionFile, const string& wordEmbFile,
		const string& charEmbFile) {
	if (optionFile != "")
  ...
  for (int idx = 0; idx < tagEmbs.size(); idx++) {
	...
	m_classifier.init(wordEmb, m_options.wordcontext, charEmb, m_options.charcontext, tagEmbs, m_labelAlphabet.size(), m_options.charhiddenSize, m_options.atomLayers, m_featAlphabet.size());

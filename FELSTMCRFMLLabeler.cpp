...
void Labeler::train(const string& trainFile, const string& devFile, const string& testFile, const string& modelFile, const string& optionFile,
  ...
  NRMat<dtype> wordEmb;
  NRMat<dtype> wordEmb_b;  
  if (wordEmbFile != "") {
    readWordEmbeddings(wordEmbFile, wordEmb);
    readWordEmbeddings(wordEmbFile, wordEmb_b);

  } else {
    wordEmb.resize(m_wordAlphabet.size(), m_options.wordEmbSize);
    wordEmb.randu(1000);
    wordEmb_b.resize(m_wordAlphabet.size(), m_options.wordEmbSize);
    wordEmb_b.randu(1000);
  }
  ...
  m_classifier.init(wordEmb, wordEmb_b, m_options.wordcontext, charEmb, m_options.charcontext, tagEmbs, m_labelAlphabet.size(), m_options.charhiddenSize, m_options.rnnHiddenSize, m_options.hiddenSize);

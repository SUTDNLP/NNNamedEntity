...
#include "LayerLSTMCRFMLLabeler.h"
...
void Labeler::train(const string& trainFile, const string& devFile, const string& testFile, const string& modelFile, const string& optionFile,
  ...
  // disable word emb tune in second LSTM
  // if (m_classifier._secondLSTM)
  //   tuneParameters();
  ...
  for (int idx = 0; idx < tagEmbs.size(); idx++) {
    std::cout << tagEmbs.size() << std::endl;
  ...
  if (!m_classifier._secondLSTM)
    m_classifier.initlast(wordEmb, m_options.wordcontext, charEmb, m_options.charcontext, tagEmbs, m_labelAlphabet.size(), m_options.charhiddenSize, m_options.rnnHiddenSize, m_options.hiddenSize);
  ...
  if (m_classifier._secondLSTM)
    std::cout << "Second LSTM, load word emb from last Hidden LSTM... " << std::endl;
  else
    std::cout << "First LSTM, load word emb from lookup table... " << std::endl;
...
int Labeler::predict(const vector<Feature>& features, vector<string>& outputs, const vector<string>& words) {
  ...
  // else
  //   m_classifier.layerPredict(features, labelIdx);
  outputs.clear();
...
void Labeler::loadModelFile(const string& inputModelFile) {
  m_options.load(inputModelFile);

}

void Labeler::writeModelFile(const string& outputModelFile) {
  m_options.writeOption(outputModelFile);

}

void Labeler::copyParameters() {
  m_classifier.copyParameters();
  
}
void Labeler::tuneParameters() {
  m_options.wordEmbFineTune = false;
  
}

void Labeler::enableSecondLSTM() {
  m_classifier._secondLSTM = true;
}
...
int main(int argc, char* argv[]) {
  ...
  std::string newtrainFile = "", newdevFile = "", newtestFile = "", newmodelFile = "";
  std::string newwordEmbFile = "", newcharEmbFile = "", newoptionFile = "";
  ...
  ah.new_named_string("newtrain", "newtrainCorpus", "named_string", "new training corpus to train a model, must when training", newtrainFile);
  ah.new_named_string("newdev", "newdevCorpus", "named_string", "new development corpus to train a model, optional when training", newdevFile);
  ah.new_named_string("newtest", "newtestCorpus", "named_string",
      "new testing corpus to train a model or input file to test a model, optional when training and must when testing", newtestFile);
  ah.new_named_string("newmodel", "newmodelFile", "named_string", "new model file, must when training and testing", newmodelFile);
  ah.new_named_string("newword", "newwordEmbFile", "named_string", "new pretrained word embedding file to train a model, optional when training", newwordEmbFile);
  ah.new_named_string("newchar", "newcharEmbFile", "named_string", "new pretrained char embedding file to train a model, optional when training", newcharEmbFile);
  ah.new_named_string("newoption", "newoptionFile", "named_string", "new option file to train a model, optional when training", newoptionFile);
  ah.new_named_string("newoutput", "newoutputFile", "named_string", "new output file to test, must when testing", newoutputFile);
  ...
  if (bTrain) {
    ...
    tagger.train(newtrainFile, newdevFile, newtestFile, newmodelFile, newoptionFile, newwordEmbFile, newcharEmbFile);
  } 
  else {
